...
const TripPosting: React.FC<TripPostingProps> = ({
  onPost,
  activeTrip,
  onNavigate,
  driverPhone,
  ...
  const [pricePerSeat, setPricePerSeat] = useState(ROUTES.SUGGESTED_PRICE_PER_SEAT || 2500);
  ...
  const handlePost = async () => {
    ...
    if (!driverPhone) {
      setErrorMsg("Driver phone missing. Please login again.");
      return;
    }
    ...
    try {
      ...
      const tripFromApi = data?.trip;

      // ✅ Map backend trip → your existing frontend Trip type
      const mappedTrip: Trip = {
        trip_id: String(tripFromApi?.id ?? Math.random().toString(36).slice(2)),
        driver_id: `driver-${driverPhone}`,
        route: `${tripFromApi?.origin ?? origin} → ${tripFromApi?.destination ?? destination}`,
        departure_time: `${departureDate}T${departureTime}:00.000Z`,
        seats_available: Number(tripFromApi?.seats_total ?? seats),
        seats_booked: Number(tripFromApi?.seats_booked ?? 0),
      ...
      setErrorMsg(e?.message || "Failed to post trip");
    } finally {
      setIsPosting(false);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full p-5 bg-white border-2 border-slate-200 rounded-2xl text-base font-black text-black focus:outline-none focus:ring-4 focus:ring-emerald-500/20 transition-all"
          />
        </div>
